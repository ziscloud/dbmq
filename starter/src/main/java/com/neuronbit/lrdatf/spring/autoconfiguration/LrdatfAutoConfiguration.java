...
public class LrdatfAutoConfiguration {
    ...
    public DefaultMQPushConsumer mqPushConsumer(ConfigProperties properties, DataSource dataSource) {
    ...
    @Bean
    public MessageConsumerScanner messageConsumerScanner(DefaultMQPushConsumer consumer) {
        return new MessageConsumerScanner(consumer);
    }

    @Bean
    @ConditionalOnBean(type = {"javax.sql.DataSource"})
    public MQProducer mqProducer(ConfigProperties properties, DataSource dataSource) {
