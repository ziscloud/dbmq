...
public class RebalancePushImplTest {
    @Spy
    private DefaultMQPushConsumerImpl defaultMQPushConsumer = new DefaultMQPushConsumerImpl(new DefaultMQPushConsumer("RebalancePushImplTest"));
    @Mock
    ...
    public void testMessageQueueChanged_CountThreshold() {
        ...
        allocateResultSet.add(new MessageQueue(topic, 0));
        allocateResultSet.add(new MessageQueue(topic, 1));
        ...
        allocateResultSet.add(new MessageQueue(topic, 2));
        doRebalanceForcibly(rebalancePush, allocateResultSet);
    ...
    private void init(final RebalancePushImpl rebalancePush) {
        ...
        when(mqClientInstance.findConsumerIdList(anyString())).thenReturn(Collections.singletonList(consumerGroup));
